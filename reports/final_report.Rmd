...
For example, when we discuss the crime rate associated with a given loan, we mean the crime rate at the time the loan approved. Thus, these quantities are known and fixed for a given loan at its approval date (see *Feature Engineering* section for further details).
...
full_data %>%
  ...
  distinct(month = month(ApprovalDate), year = year(ApprovalDate), 
           market, value) %>%
...
full_data %>%
  distinct(month = month(ApprovalDate), year = year(ApprovalDate), 
           BorrState, unemploy_rate) %>%
  mutate(date = ISOdate(year, month, 1),
         unemploy_rate = unemploy_rate / 100) %>%
  dmap_at("date", as.Date) %>%
...
bind_rows(getTrainPerf(elastic.fit), 
  ...
  geom_point() +
  geom_text_repel() +
  labs(x = "Training Sensitivity", y = "Training AUC", 
...
trainResults %>%
  ...
  labs(x = "False Positive Fraction", y = "True Positive Fraction",
       title = "Training ROC Curves by Model Type") +
...
```{r distribution, eval=T,echo=F,fig.height=3.5,fig.width=3.5, fig.align='center'}
descrip_status = table(status_m)
names(descrip_status) = c("right_censored","paid_off","defaulted")
descrip_status %>%
  as_data_frame() %>%
  plyr::rename(replace = c("Var1" = "loan_status")) %>%
  mutate(percentage = n/sum(n)) %>%
  ggplot(mapping = aes(x = reorder(loan_status, -percentage), y = percentage)) +
  geom_col() +
  scale_y_continuous(breaks = seq(0,0.8,0.1),
                     labels = percent) +
  labs(x = "Loan Status",
       y = "Percent of Training Data",
       title = "Loans in Training Data by Status")
```	
...
We fit models varying $\alpha$ and $\lambda$ in the above penalty. The best model was determined using a goodness of fit measure defined by the `glmnet` package in R called `dev.ratio`. It is a measure of the difference between the maximized likelihood of the null model (no covariates) and that of the fit model: 
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=325pt,height=620pt]{../studies/heatmap.pdf}
  \caption{`dev.ratio` values}
  \label{fig:sub1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=325pt,height=620pt]{../studies/ftrs_heatmap.pdf}
...
**Recall that at the beginning of the report we made note that all appended time series data variables were matched with our data at approval date. Thus, for example, the crime rate variable in a given feature vector is the crime rate at the time the loan was approveed.** The below figures show the out of sample performance of the one and five year ahead
probabilities estimated by the Cox model for active loans as of February, 1 2010. This date was arbitrarily selected, but is the date at which we will construct the 500 loan portfolio in the following section:   
...
```{r}
read_rds("../data/alex_rfe_results.rds") %>%
  ggplot() +
  labs(x = "Number of Variables",
...
```{r}
rf.fit = read_rds("../data/alex_rf_fit.rds")

rf.fit$results %>%
  ggplot(mapping = aes(x = mtry, y = RMSE)) +
  geom_point() +
  geom_line() +
...
```{r echo=FALSE}
realised_loss_file = '../data/realised_losses.rds'
realised_losses = read_rds(realised_loss_file)

ggplot(data = one_year_default_result, aes(one_year_default_result$Percentage)) +
  geom_histogram(position = "identity") +
  scale_x_continuous(labels = scales::percent) + 
  scale_y_continuous(labels = scales::comma) + 
  geom_vline(xintercept = VaR_list[3][[1]]$Mean, linetype = "longdash", show.legend = T, color="darkgreen") + 
  geom_vline( xintercept = ETL_list[3][[1]]$Mean, linetype = "longdash", show.legend = T, color="red") + 
  geom_vline( xintercept = realised_losses$loss_1yr/100, linetype = "longdash", show.legend = T, color= "darkblue") +
  labs(x = "Total Percentage Loss on Porfolio", y = "Count", 
       title = "One-Year Expected Loss Distribution from 500 Loan Portfolio") +
  annotate("text", x = c(0.033,0.033, 0.033), y = c(11000,10500,10000), 
           label =c("VaR 95%", "ETL 95%", "Realised Loss"), colour = c("darkgreen", "red", "darkblue"))

ggplot(data = five_year_default_result, aes(five_year_default_result$Percentage)) +
  geom_histogram(position = "identity") +
  scale_x_continuous(labels = scales::percent) + 
  scale_y_continuous(labels = scales::comma) + 
  geom_vline(xintercept = VaR_list[5][[1]]$Mean, linetype = "longdash", show.legend = T, color="darkgreen") + 
  geom_vline( xintercept = ETL_list[5][[1]]$Mean, linetype = "longdash", show.legend = T, color="red") + 
  geom_vline( xintercept = realised_losses$loss_5yr/100, linetype = "longdash", show.legend = T, color= "darkblue") + 
  labs(x = "Total Percentage Loss on Porfolio", y = "Count", 
       title = "Five-Year Expected Loss Distribution from 500 Loan Portfolio") +
  annotate("text", x = c(0.095,0.095, 0.095), y = c(11500,11000,10500), 
           label =c("VaR 95%", "ETL 95%", "Realised Loss"), colour = c("darkgreen", "red", "darkblue"))
```
...
##Theo Vadpey
Wrote `cox_processing.R`,`cox_models.R`,`cox_diagnostic_functions.R`,`cox_refit_best_model.R`,`cox_surv_probs.R`. Wrote section on Cox modeling. Generated 1yr, 5yr default probabilities on test data. 
...
##Ben Etringer
Computed the tranche loss distributions and the risk analysis (code provided inline). Also researched and cleaned the external features that were used in the model (generating dummy indicator features as needed).
