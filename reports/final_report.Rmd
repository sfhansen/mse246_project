...
and 14 other volatility market indices. **It is important to note the way in which we appended these variables to our data. For a given loan, we included the GDP, crime rate, unemployment rate, etc, at the time the loan was approved. So when we speak of the crime rate (for example) for a given loan, we mean the crime rate at the time the loan approved. Thus, these quantities are known and fixed for a given loan at approval.**
